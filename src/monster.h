...
class Monster final : public Creature {
	public:
		...
		uint16_t auraEffect() const {
			return mType->info.auraEffect;
		}
		std::string shaderEffect() const {
			return mType->info.shaderEffect;
		}
		uint16_t wignsEffect() const {
			return mType->info.wignsEffect;
		}
		uint16_t rayosEffect() const {
			return mType->info.rayosEffect;
		}

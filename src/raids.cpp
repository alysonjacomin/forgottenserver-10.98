...
bool Raids::startup()
{
	...
	checkRaidsEvent = g_scheduler.addEvent(createSchedulerTask(CHECK_RAIDS_INTERVAL * 1000, [this]() {
		checkRaids();
	}));
...
void Raid::executeRaidEvent(RaidEvent* raidEvent)
{
	if (raidEvent->executeEvent()) {
		...
		if (newRaidEvent) {
			...
			nextEventEvent = g_scheduler.addEvent(createSchedulerTask(ticks, [=, this]() {
